...
3. **Preset video của downloader cần ffmpeg (Phase 12) — ĐÃ TEST 2026-09-04, CHỜ t CHỐT.** Kết quả chi tiết ở `plan/MANUAL_TESTS_TODO.md` mục 1. Tóm tắt: tab Video chỉ có stream video-only (mp4 câm, Summary vẫn báo thành công); preset `bestvideo+bestaudio` không ffmpeg → 2 file rời (.mp4 video-only + .webm opus); yt-dlp không có JS runtime → YouTube không còn stream muxed nào → **B một mình = tab Video trống với YouTube**. Lựa chọn:
   - A: đóng gói ffmpeg binary (arm64 + x86_64, +vài chục MB), đặt `ffmpeg_location` cho yt-dlp.
   - B: chỉ lọc `vcodec != none && acodec != none` (`video_only_formats`) — vẫn nên làm, nhưng với YouTube sẽ không còn dòng nào.
   - C (đề xuất): B + selector `bestvideo[ext=mp4][vcodec^=avc1]+bestaudio[ext=m4a]` (yt-dlp để lại `.fNNN.mp4` + `.fNNN.m4a`), rồi `DownloadForegroundService` ghép 2 file bằng `MediaMuxer` (tái dùng cách của `AudioExtractor.kt`), xoá 2 file rời. Không cần ffmpeg, không tăng dung lượng; giới hạn codec H.264/AAC để MediaMuxer chắc chắn ghép được.
   - Rủi ro chung: yt-dlp đã deprecate extraction YouTube không JS runtime → tương lai có thể hỏng hẳn, không chỉ thiếu format; Chaquopy không có deno/node. Cần theo dõi ở Phase 12/13.
...
**Checklist:**
- [x] `queue_end_state` — `lib/providers/player_provider.dart:136` (high) — `_onPlaylistEnded` chỉ xử lý shuffleLoop; với repeat tắt just_audio giữ `playing = true` ở `completed` (mini player icon pause, đĩa xoay, bấm play không kêu — `AudioPlayer.java:965-980` không seek). **ĐÃ CHỐT, spec:** khi `completed`, `_repeatMode == none`, `_originalQueue` không rỗng: (1) ghi `generation = _loadGeneration`, `await _handler.pause()` TRƯỚC mọi seek/load (vì `playing` đang true, load xong sẽ tự phát); nếu `_loadGeneration` đã đổi thì return (người dùng vừa bấm bài mới). (2) Nếu `_shuffleEnabled`: `_buildShuffledQueueTrueRandom(startIndex: Random().nextInt(_originalQueue.length))`, `_currentPlayIndex = 0`, `_currentSong = _playQueue[0]`, `_historyStack.clear()`, `notifyListeners()`, `await _loadQueueToHandler(0)` — KHÔNG gọi play (giống nhánh shuffleLoop nhưng dừng). (3) Nếu không shuffle: `await _seekToIndex(0, recordHistory: false)`, `_historyStack.clear()`. Kết quả: nút hiện "play", thanh về 0:00 ở bài đầu (danh sách mới nếu shuffle), bấm play thì nghe từ đầu. Test trong `player_provider_test.dart` với fake gateway phát `ProcessingState.completed`: pause được gọi trước load/seek, index về 0, `isPlaying == false`, shuffle bật thì queue đổi thứ tự và được load lại, shuffleLoop giữ hành vi cũ (phát tiếp).
- [x] `nav_stack_duplicate` — `lib/screens/onboarding_screen.dart:111` (high) — `_navigateHome` dùng `pushReplacement` trong khi 6 nơi gọi (`welcome_screen:134`, `home_screen:322,738`, `library_screen:215`, `profile_screen:104`, `downloader_gateway_screen:68`) đều `push` → lần đầu stack `[Welcome, Home]` (back về Welcome), quét lại tạo Home thứ hai. Lần đầu: `pushAndRemoveUntil(home, (_) => false)`; quét lại: `pop()` về màn gọi.
- [x] `empty_shuffle_loop` — `lib/screens/playlist_screen.dart:533-571` + `player_provider.dart:221-226` (high) — Hàng nút Shuffle Loop nằm ngoài guard `songs.isNotEmpty`; `enableShuffleLoop([])` vẫn gán `_repeatMode` và mở Now Playing trống. Đưa vào guard/disable khi rỗng; `enableShuffleLoop` return sớm khi rỗng (đóng luôn ghi chú Phase 1 của AUDIT_PLAN).
- [x] `double_push_now_playing` — `playlist_screen.dart:498-524,542-568`, `album_detail_screen.dart:86-91,110-115`, `artist_detail_screen.dart:94-99,116-121` (medium) — Shuffle/Shuffle Loop `await playSongsShuffled()` xong mới push Now Playing, nút vẫn bấm được → double-tap ra 2 màn Now Playing. Push ngay như "Phát tất cả" (currentSong đã set trước await đầu) hoặc cờ busy.
- [x] `play_next_semantics` — `lib/widgets/music_list_tile.dart:361-389` + `player_provider.dart:343-348` (medium) — "Phát tiếp theo" gọi `addToQueue` (thêm cuối). **ĐÃ CHỐT, spec:** (1) `PlayerProvider.insertNext(SongItem)`: chèn vào `_playQueue` tại `_currentPlayIndex + 1`, vào `_originalQueue` ngay sau bài hiện tại (tìm theo id; không thấy thì thêm cuối); gateway thêm `insertSongAt(int index, SongItem song)` (handler: `_playlist.insert(index, AudioSource.uri(...))` + `_currentSongs.insert`, tách helper tạo `AudioSource` dùng chung với `loadSongs`/`addSongToQueue`); nếu chưa có bài nào đang phát thì `playSongs([song])` như hiện tại. Cập nhật 4 fake gateway trong `test/` (accessibility ×3, player_provider_test). (2) Menu bài hát (`music_list_tile.dart:361`): "Phát tiếp theo" gọi `insertNext`, snackbar `AppStrings.willPlayNext(title)` mới ('Sẽ phát tiếp theo: "$title"'); thêm mục mới ngay dưới: `AppStrings.addToQueue` ('Thêm vào hàng chờ', icon `Icons.playlist_add_rounded`) gọi `addToQueue` với snackbar `addedToQueue(title)` hiện có. Grep `AppStrings.playNext` để áp cùng pattern cho menu khác nếu có. (3) Test `player_provider_test.dart`: insertNext khi đang phát giữa queue → bài mới ở `queue[currentIndex + 1]`, gateway nhận đúng index; shuffle bật → `_originalQueue` cũng có bài; chưa phát → playSongs; addToQueue vẫn thêm cuối.
- [x] `play_count_source` — `lib/providers/music_provider.dart:360-367` + 11 chỗ gọi `onSongPlayed` trên UI (medium) — Auto-next, Next/Prev, queue, shuffle không được ghi "Nghe gần đây/Nghe nhiều". Ghi 1 lần từ PlayerProvider khi currentSong đổi (callback inject từ main.dart), xoá 11 lời gọi UI. Chú ý mục `prefs_write_race` (Phase 11) khi bấm Next liên tục.
- [x] `selection_mode_back` — `lib/screens/library_screen.dart` (low) — Không có `PopScope`: back khi đang chọn thoát luôn màn. `PopScope(canPop: !_isSelecting, onPopInvokedWithResult: … _exitSelecting())`.
- [x] `selection_mode_scroll_jump` — `lib/screens/library_screen.dart:320-348` (medium) — Vào/ra chế độ chọn đổi child của `Expanded` giữa `_FadeTabBarView` và `_SongsTab` trần → ListView dựng lại, nhảy về đầu. Giữ 1 element `_SongsTab` (truyền `isSelecting` xuống) hoặc `PageStorageKey`.
- [x] `search_clear_incomplete` — `lib/screens/library_screen.dart:1362-1365` (medium) — "Xóa tìm kiếm" ở empty state chỉ `setLibrarySearchQuery('')`, `_searchCtrl` vẫn giữ chữ, gõ lại cùng chữ bị bỏ qua. Truyền `onClear` xoá controller như `home_screen.dart:139-143`.
- [x] `rescan_fixed_delay` — `lib/screens/onboarding_screen.dart:82` (medium) — Mỗi lần quét lại chờ cứng 5s intro + 2s kết quả. Bỏ intro khi `music.hasScannedOnce`, overlap thời gian tối thiểu với scan như `AppStartupService`.
- [x] `seek_during_load` — `lib/providers/player_provider.dart:473-481` (low, ghi chú Phase 1 AUDIT_PLAN) — `_seekToIndex` không qua `_loadChain`; bấm Next đúng lúc queue đang load có thể seek vào playlist chưa build xong. Chờ `_loadChain` trước khi seek.
...
- (Phase 10, 2026-09-04) Kiểm chứng: `flutter analyze` sạch; `flutter test` 102/102 (86 + 16 test mới trong `player_provider_test.dart`: hết hàng chờ ×4, play next ×4, onSongPlayed ×4, seek khi đang load ×2, currentSong đặt trước await, shuffle loop rỗng). Chưa check tay trên máy thật: làm theo mục "Check tay" cuối Phase 10 và `MANUAL_TESTS_TODO.md` mục 3.
- (Phase 10, 2026-09-04) Lệch spec nhỏ, cần biết: "Thêm vào hàng chờ" dùng `Icons.queue_rounded` thay vì `playlist_add_rounded` vì icon đó đã là "Thêm vào playlist" ngay phía trên trong cùng menu. `rescan_fixed_delay`: quét lại/thử lại vẫn giữ tối thiểu 1s (tránh nháy thanh tiến trình), intro 5s chỉ lần quét đầu, đều chạy song song với scan; 2s hiện kết quả giữ nguyên.
- (Phase 10, 2026-09-04) `play_count_source`: đếm khi `playSongs`/`playSongsShuffled` (kể cả bấm lại bài đang phát, như UI cũ), đổi index (auto-next/Next/Prev/queue), xoá bài đang phát khỏi hàng chờ, shuffle loop quay vòng; KHÔNG đếm khi hết hàng chờ dừng về bài đầu, toggle shuffle, reorder. `prefs_write_race` (Phase 11) vẫn còn khi bấm Next liên tục.
- (Phase 10, 2026-09-04) `double_push_now_playing`: chọn hướng "push ngay": `playSongs`/`playSongsShuffled` đặt `currentSong` trước await đầu (bỏ repeat-one chỉ đổi cờ đồng bộ, engine cập nhật sau), `enableShuffleLoop` đặt `_repeatMode` trước. 2 nút Shuffle/Shuffle Loop trong `playlist_screen` dùng `playerRoute()` từ `widgets/library/player_route.dart` (Phase 14 `dup_now_playing_route` gộp nốt các bản inline còn lại).
- (Phase 10, 2026-09-04) `nav_stack_duplicate`: `OnboardingScreen(isFirstRun: true)` chỉ từ Welcome; 5 chỗ quét lại `pop()` về màn gọi; fallback `pushAndRemoveUntil(Home)` khi không pop được. `selection_mode_back`: PopScope nằm trong LibraryScreen nhúng ở Home (Home không có PopScope riêng) → back khi đang chọn chỉ thoát chế độ chọn.
- (Test tay mục 1, 2026-09-04) Emulator: sau 2 lần `am force-stop` + `am start` liên tiếp (giữa các lần tải), app treo khi khởi động — ANR "Input dispatching timed out (Application does not have a focused window)", màn đen, `Skipped 30 frames` trước đó; lần khởi động đầu sau khi cài APK thì bình thường. Chưa điều tra nguyên nhân (nghi `DownloadForegroundService`/Chaquopy chạy trên main thread khi khôi phục record cũ, hoặc emulator quá tải vì host đang build). Cần xem lại ở Phase 12 (`notifier_error_unlogged`/khởi động service).
- (Test tay mục 1, 2026-09-04) Summary sau MỖI lần tải 1 video đều báo "Tất cả 2 video đã được tải xuống" (Tổng 2 / Thành công 2) — đếm cả record cũ (Rick Astley 28/08) do service khôi phục; kể cả sau khi restart app. Ghi để Phase 12 xem cùng `summary_nav_blank`.
- (Test tay mục 1, 2026-09-04) `video_only_formats`: ngoài stream DASH video-only, tab Video còn lẫn format 233/234 (HLS audio-only "Default, low/high", `vcodec=none`, `acodec=null`) → bộ lọc phải kiểm tra cả vcodec lẫn acodec khác `none`/null.
- (Test tay mục 1, 2026-09-04) Cách kiểm tra file có tiếng không cần ffprobe: pull file, đọc atom `moov/trak/mdia/hdlr` (script `mp4tracks.py` trong session, 50 dòng Python).
