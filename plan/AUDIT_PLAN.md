...
**Checklist:**
- [x] `error_state_missing` — `lib/features/downloader/screens/analyze/analyze_screen.dart:54` (medium) — Lỗi khởi động hiển thị raw exception $e cho user, không có nút thử lại, và nút Phân tích kẹt ở 'Đang khởi động...' (isLoading = !_…
- [x] `empty_state_missing` — `lib/features/downloader/screens/playlist_picker/playlist_picker_screen.dart:286` (low) — Khi playlist không có video hợp lệ hoặc từ khoá tìm kiếm không khớp, _EntryList render trống hoàn toàn — user tưởng app lỗi/đang t…
- [x] `feature_structure_drift` — `(cross-file)` (low) — Repo dùng 2 quy ước cùng lúc: phần lõi là layered-flat (`lib/screens`, `lib/providers`, `lib/services`, `lib/widgets`, `lib/models…
- [x] `form_field_ergonomics` — `lib/features/downloader/screens/analyze/analyze_screen.dart:633` (low) — TextField khai báo action 'Go' nhưng không có onSubmitted, bấm Go trên bàn phím không phân tích gì — user phải đóng bàn phím rồi t…
- [x] `form_field_ergonomics` — `lib/screens/now_playing_screen.dart:1677` (low) — Form 2 trường (Tên bài hát, Nghệ sĩ) không có textInputAction next/done và không có onSubmitted → nhấn Enter trên bàn phím không c…
- [x] `hardcoded_style` — `lib/features/downloader/screens/analyze/analyze_screen.dart:857` (low) — Màu ngữ nghĩa (error 0xFFFF3B30 x4, success 0xFF34C759, warning 0xFFFF9F0A) hardcode rải rác thay vì token trong AppColors — đổi t…
- [x] `hardcoded_style` — `lib/features/downloader/screens/download/download_screen.dart:136` (low) — Màu trạng thái queued/done/error (0xFFFF9F0A, 0xFF34C759, 0xFFFF3B30) hardcode lặp lại ~12 lần trong 4 widget khác nhau thay vì to…
- [x] `hardcoded_style` — `lib/features/downloader/screens/downloader_gateway_screen.dart:161` (low) — Màu trạng thái online/offline, màu cảnh báo 0xFFFF9500 (dòng 310), gradient 0xFF5C6BC0 (dòng 136) và màu disabled 0xFF2A2A2A (dòng…
- [x] `hardcoded_style` — `lib/features/downloader/screens/format/format_screen.dart:338` (low) — Màu warning 0xFFFF9500 (x4), success 0xFF34C759, warning 0xFFFF9F0A hardcode trong widget thay vì token AppColors — 2 sắc cam khác…
- [x] `hardcoded_style` — `lib/features/downloader/screens/summary/summary_screen.dart:195` (low) — Màu success/error (0xFF34C759, 0xFF30D158, 0xFFFF3B30 x4) hardcode rải rác thay vì token AppColors — không đồng bộ với các màn khá…
- [x] `hardcoded_style` — `lib/features/downloader/widgets/network_status_badge.dart:76` (low) — Màu online/offline viết cứng (trùng với downloader_gateway_screen) và fontSize 11 magic number thay vì token AppColors / textTheme…
- [x] `hardcoded_style` — `lib/screens/album_detail_screen.dart:43` (low) — Icon back ghi cứng Colors.white trong SliverAppBar pinned có backgroundColor c.background: khi cuộn thu gọn ở theme Light, icon tr…
- [x] `hardcoded_style` — `lib/screens/library_screen.dart:456` (low) — Menu sắp xếp dùng token tĩnh AppColors (hằng dark-only) trong khi phần còn lại của file dùng context.appColors theo theme; ở light…
- [x] `hardcoded_style` — `lib/screens/now_playing_screen.dart:391` (low) — Nhiều màu raw (Colors.black.withValues 0.55/0.75, Colors.white54/white38/white24, Slider activeTrackColor: Colors.white) rải rác d…
- [x] `hardcoded_style` — `lib/screens/welcome_screen.dart:99` (low) — Tiêu đề 'Muzic' ghi cứng Colors.white trên Scaffold backgroundColor c.background: ở theme Light chữ trắng trên nền sáng không đọc …
- [x] `hardcoded_ui_strings` — `lib/features/downloader/screens/downloader_gateway_screen.dart:93` (low) — Toàn bộ copy của màn hình ('Tải nhạc từ URL', 'Quét lại thư viện', 'Cần kết nối mạng để tải nhạc', các dòng Lưu ý...) nằm inline t…
- [x] `hardcoded_ui_strings` — `lib/features/music_visual/widgets/visual_mode_selector_sheet.dart:97` (low) — Tiêu đề, mô tả, nút 'Áp dụng', badge 'Hiện tại' và mô tả từng mode viết inline trong widget; không có file strings tập trung.
- [x] `hardcoded_ui_strings` — `lib/screens/album_detail_screen.dart:198` (low) — Nhãn nút, chuỗi đếm bài và nội dung dialog Shuffle Loop ghi cứng inline (trùng nguyên văn với ArtistDetailScreen, nên càng cần gom…
- [x] `hardcoded_ui_strings` — `lib/screens/artist_detail_screen.dart:72` (low) — Nhãn nút, tiêu đề section, nội dung dialog Shuffle Loop và chuỗi đếm '$songCount bài hát · $albumCount album' đều ghi cứng inline.
- [x] `hardcoded_ui_strings` — `lib/screens/hidden_songs_screen.dart:31` (low) — Chuỗi giao diện viết cứng trong widget, không có file strings tập trung.
- [x] `hardcoded_ui_strings` — `lib/screens/home_screen.dart:359` (low) — Chuỗi giao diện (lời chào, hint, tiêu đề section, empty state) viết cứng trong widget, không có file strings tập trung.
- [x] `hardcoded_ui_strings` — `lib/screens/library_screen.dart:260` (low) — Toàn bộ chuỗi giao diện (tiêu đề, hint, snackbar, dialog) viết cứng trong widget, không có file strings tập trung nên không thể đị…
- [x] `hardcoded_ui_strings` — `lib/screens/now_playing_screen.dart:423` (low) — Hàng chục chuỗi tiếng Việt (nhãn menu, tiêu đề sheet, snackbar, dialog) viết inline trong widget; dự án chưa có i18n hay file stri…
- [x] `hardcoded_ui_strings` — `lib/screens/onboarding_screen.dart:289` (low) — Mọi chuỗi trạng thái quét, lỗi, nút 'Thử lại'/'Mở cài đặt' và 20 câu quote đều ghi cứng inline, không có file strings trung tâm.
- [x] `hardcoded_ui_strings` — `lib/screens/online_screen.dart:34` (low) — Chuỗi giao diện viết cứng trong widget, không có file strings tập trung.
- [x] `hardcoded_ui_strings` — `lib/screens/playlist_screen.dart:381` (low) — Mọi chuỗi giao diện (nút, dialog, empty state) viết cứng tiếng Việt trong widget, không có file strings tập trung.
- [x] `hardcoded_ui_strings` — `lib/screens/profile_screen.dart:130` (low) — Toàn bộ chuỗi UI ghi cứng; đặc biệt số phiên bản '1.0.0' lệch với pubspec (2.0.0+19) và hộp About viết sai tên app 'Muzizc Audio'.…
- [x] `hardcoded_ui_strings` — `lib/screens/welcome_screen.dart:128` (low) — Tagline và nhãn CTA ghi cứng inline, không có file strings trung tâm.
- [x] `hardcoded_ui_strings` — `lib/widgets/add_to_playlist_sheet.dart:88` (low) — Chuỗi giao diện (tiêu đề, hint, snackbar, dialog) viết cứng trong widget, không có file strings tập trung.
- [x] `hardcoded_ui_strings` — `lib/widgets/app_bottom_navigation.dart:73` (low) — Nhãn tab ghi cứng và lặp 2 lần (glass + normal), trộn tiếng Anh 'Home' với 'Trực tuyến'/'Thư viện' tiếng Việt.
- [x] `hardcoded_ui_strings` — `lib/widgets/bottom_nav_style_selector_sheet.dart:94` (low) — Tiêu đề, mô tả, nhãn 'Áp dụng', 'Hiện tại' và subtitle option ghi cứng inline.
- [x] `hardcoded_ui_strings` — `lib/widgets/mini_player.dart:228` (low) — Chuỗi dialog và nhãn semantics ('Dừng phát nhạc?', 'Hàng chờ hiện tại sẽ bị xóa.', 'Đóng trình phát', 'Bài trước'...) viết inline;…
- [x] `hardcoded_ui_strings` — `lib/widgets/music_list_tile.dart:337` (low) — Nhãn menu ngữ cảnh, SnackBar và bảng 'Thông tin bài hát' ghi cứng inline.
- [x] `hardcoded_ui_strings` — `lib/widgets/theme_selector_sheet.dart:101` (low) — Tiêu đề, mô tả, nhãn 'Áp dụng', 'Hiện tại', hint và 3 subtitle theme ghi cứng inline.
- [x] `missing_semantics` — `lib/features/downloader/screens/analyze/analyze_screen.dart:519` (low) — Nút chọn thư mục và nút back chỉ có icon, không tooltip/Semantics label — TalkBack đọc 'button' không rõ chức năng.
- [x] `missing_semantics` — `lib/features/downloader/screens/downloader_gateway_screen.dart:388` (low) — Hai nút chính của màn hình là GestureDetector bọc Container, không có Semantics(button: true, enabled:) nên TalkBack không nhận ra…
- [x] `missing_semantics` — `lib/features/downloader/widgets/primary_icon_button.dart:68` (low) — Nút chỉ có icon, không có tooltip hay Semantics label nên TalkBack chỉ đọc 'button' không rõ chức năng (paste/clear/analyze...).
- [x] `missing_semantics` — `lib/screens/album_detail_screen.dart:41` (low) — 2 control chỉ có icon không có nhãn a11y: nút back (không tooltip) và nút info Shuffle Loop (GestureDetector bọc Icon, dòng 117).
- [x] `missing_semantics` — `lib/screens/artist_detail_screen.dart:46` (low) — 2 control chỉ có icon không có nhãn a11y: nút back (không tooltip) và nút info Shuffle Loop (GestureDetector bọc Icon, dòng 119) —…
- [x] `missing_semantics` — `lib/screens/now_playing_screen.dart:604` (low) — Bìa album (chạm để lật sang lời bài hát) và _LyricsView (chạm để lật về) là GestureDetector trần không có Semantics button/label; …
- [x] `missing_semantics` — `lib/screens/playlist_screen.dart:248` (low) — FAB tạo playlist, nút info (dòng 492), nút gỡ bài (dòng 652) là GestureDetector bọc Icon không có Semantics/tooltip; các IconButto…
- [x] `missing_semantics` — `lib/screens/profile_screen.dart:493` (low) — IconButton quay lại chỉ có icon, không có tooltip nên TalkBack đọc là 'Button' không rõ chức năng.
- [x] `tap_target_small` — `lib/features/downloader/screens/analyze/analyze_screen.dart:149` (low) — Nút back là GestureDetector 36x36 và _ActionIconButton (Dán/Xóa) cũng 36x36 — dưới ngưỡng 44-48dp, dễ bấm trượt (2 chỗ trong file)…
- [x] `tap_target_small` — `lib/features/downloader/screens/download/download_screen.dart:581` (low) — _TinyButton (Hủy/Thử lại/Xóa) là GestureDetector padding 10x5 với icon 13 + text 12 → cao ~26dp, dưới ngưỡng 44dp; nút Xóa/Hủy cạn…
- [x] `tap_target_small` — `lib/features/downloader/screens/playlist_picker/playlist_picker_screen.dart:253` (low) — Nút 'Chọn tất cả / Bỏ chọn tất cả' là GestureDetector text 12px padding 6 dọc → cao ~28dp, dưới ngưỡng 44dp.
- [x] `tap_target_small` — `lib/features/downloader/screens/summary/summary_screen.dart:338` (low) — Nút 'Thử lại tất cả' là GestureDetector padding 10x4 với text 12px → cao ~24dp, dưới ngưỡng 44dp.
- [x] `tap_target_small` — `lib/screens/library_screen.dart:341` (low) — Nút xoá tìm kiếm là GestureDetector bọc Icon 18px không padding nên vùng chạm chỉ ~18x18dp, rất khó bấm trúng.
- [x] `tap_target_small` — `lib/screens/now_playing_screen.dart:826` (low) — Nút đóng pill bar chỉ 16 + 6*2 = 28dp; các action icon trong _buildActionIcon 20 + 10*2 = 40dp; link tên album ở _TopBar (chữ 12sp…
- [x] `tap_target_small` — `lib/screens/playlist_screen.dart:576` (low) — Nút 'Thêm bài' là GestureDetector bọc Row icon 18px + chữ 13px không padding (cao ~20dp); nút gỡ bài trong danh sách (dòng 652) cũ…
- [x] `tap_target_small` — `lib/widgets/add_to_playlist_sheet.dart:156` (low) — Nút xoá tìm kiếm là GestureDetector bọc Icon 18px không padding nên vùng chạm chỉ ~18x18dp.
...
- (Phase 7, 2026-08-28) Theme: thêm token ngữ nghĩa `success/warning/error`, `successGradient`, `disabledGradient`, `scrimStrong` vào `AppColorsData` (3 preset; light dùng warning `FF9500` cam iOS light) và bản static `AppColors`. Đổi hình ảnh nhỏ do tokenise: nút "Quét lại thư viện" ở gateway dùng `c.randomMixGradient` (cyan→tím) thay gradient indigo `5C6BC0→9B5CBF` ghi cứng; gradient disabled ở gateway giờ là token `disabledGradient` (`444444/333333`, trước `2A2A2A/222222`, bấm không được nên ở opacity 0.45 khó thấy khác); `PrimaryButton/PrimaryIconButton` disabled cũng dùng token này (ghi chú Phase 3). Cố ý giữ nguyên: `Colors.black.withValues` trong `BoxShadow` (bóng, không phải màu theme) và `Colors.white` trong `ShaderMask` của header analyze (mask, chỉ cần alpha).
- (Phase 7, 2026-08-28) Còn ~27 `Colors.white/black` ở file NGOÀI checklist (home_screen 5 — thẻ Truy cập nhanh; app_theme 5 — `onPrimary` v.v.; music_list_tile, mini_player, add_to_playlist_sheet 2 mỗi file; splash, profile, theme_switch_wrapper, app_bottom_navigation 1). Đa số là chữ/icon trắng trên gradient (→ `c.onPlayer`) — quét nốt khi rảnh, không nằm trong finding nào.
- (Phase 7, 2026-08-28) `lib/core/app_strings.dart` (`abstract final class AppStrings`): chuỗi tĩnh `static const`, chuỗi có tham số là hàm (`songCount(n)`, `addedToPlaylist(name)`…), 20 câu quote onboarding thành `onboardingQuotes`. Đã quét 19 file trong checklist + 13 file `lib/widgets/now_playing/*` (tách từ now_playing_screen ở Phase 4a). CỐ Ý để inline: key map (`'title'`, `'artist'`), value popup menu (`'edit'`, `'delete'`…), định dạng số/`'${song.artist} · ${song.album}'`. 5 màn downloader còn lại (analyze/format/download/summary/picker) KHÔNG trong checklist nên vẫn inline; chuỗi MỚI thêm ở đó (tooltip, empty/error state) đã dùng AppStrings — `AppStrings.clear` ('Xóa') trùng giá trị `delete`, gộp khi quét downloader. Đổi copy duy nhất: tab 'Home' → 'Trang chủ' (finding nêu lẫn Anh/Việt); 2 test theo `AppStrings.tabHome`.
- (Phase 7, 2026-08-28) profile: version đọc từ `package_info_plus` (mới thêm ^9.0.1) qua `static final Future<PackageInfo>` trong `ProfileScreen` (cache cả vòng đời app, subtitle hiện '…' 1 frame đầu); About dialog sửa 'Muzizc' → `AppStrings.appName`/`copyright`. `flutter pub add` báo lỗi "Building with plugins requires symlink support / Developer Mode" ở bước tạo symlink plugin desktop nhưng pubspec/lock/.flutter-plugins-dependencies đều đã cập nhật; Android build + màn Hồ sơ chạy bình thường. Muốn hết cảnh báo thì bật Developer Mode Windows (`start ms-settings:developers`).
- (Phase 7, 2026-08-28) error_state analyze: lỗi init thư mục hiện `AppStrings.initFailed` + nút Thử lại (`ref.invalidate(downloadOutputDirectoryProvider)`), nút Phân tích chỉ spin khi provider `isLoading`. Chuỗi lỗi gốc không còn hiện lên UI và cũng KHÔNG được log ở đâu (notifier `build()` chỉ throw) — nếu cần debug thì thêm `debugPrint` trong `DownloadOutputDirectoryNotifier.build`.
- (Phase 7, 2026-08-28) tap_target: một số layout cao hơn vì vùng bấm ≥44dp — hàng nút Hủy/Thử lại/Xóa trong task card download (~26→44dp); toolbar 'Chọn tất cả' của picker đã giảm padding dọc 10→4 để bù; hàng '{n} bài hát / Thêm bài' ở playlist detail giảm padding trên 12→4; icon trong pill bar Now Playing 40→48dp (5×48 = 240 trong bề rộng 280). Link album ở TopBar: cả khối 'ĐANG PHÁT / tên album' (48dp) giờ là nút mở sheet album thay vì riêng dòng 18dp — Semantics hint 'Xem các bài trong album'.
- (Phase 7, 2026-08-28) missing_semantics: `PrimaryIconButton.semanticLabel` là tham số BẮT BUỘC (Semantics + Tooltip) — chỉ có 1 call site (nút thư mục analyze) + 1 test. `_GatewayButton` dùng `Semantics(excludeSemantics: true, onTap:…)` để TalkBack đọc 1 node "label, hint/lý do disabled" thay vì đọc rời từng Text. `LyricsView` đặt Semantics BÊN TRONG GestureDetector với `explicitChildNodes: true` để nút "Lời bài hát, chạm để xem bìa" không gộp toàn bộ lời vào label.
- (Phase 7, 2026-08-28) `feature_structure_drift`: chỉ ghi nhận theo hướng dẫn phase. Đề xuất trong report: feature-first + `lib/core/{theme,widgets,utils}`; phase này đã tạo `lib/core/` (app_strings.dart) — bước đầu; `lib/theme`, `lib/widgets/glass_container.dart`, `lib/utils/duration_format.dart` chưa dời. Làm khi có session riêng (đụng import của gần như mọi file).
- (Phase 7, 2026-08-28) Ghi chú cũ còn mở: badge "Online" đè nút "Xóa xong" ở download screen (Phase 3) — không thuộc finding nào, chưa sửa; `updateSongMeta` không đồng bộ vào queue/currentSong (Phase 4a) — chưa sửa; Phase 4b (format_screen) / 4c (library_screen) chưa làm. Chưa chạy auditz scan/baseline vì 4b/4c chưa xong ("sau khi xong tất cả").
- (Phase 7, 2026-08-28) Kiểm chứng: `dart analyze` sạch sau từng commit; `flutter test` 84/84 (2 test đổi 'Home' → `AppStrings.tabHome`, 1 test thêm `semanticLabel`). Chạy bản debug trên emulator Pixel_9_Pro (theme Light): Home ('Trang chủ', chuỗi từ AppStrings) → Hồ sơ hiện 'Muzicz Audio v2.0.0' từ package_info_plus → Gateway đổi theo theme Light (đóng ghi chú Phase 3) → Analyze: nút back/Dán 36dp trong vùng 48dp, gõ URL + phím Go tự phân tích → Format → Download (màu trạng thái từ token, nút Hủy/Xóa 44dp) → Summary (successGradient). Now Playing: layout top bar không đổi, bấm khối 'ĐANG PHÁT / album' mở sheet album, pill mở đủ 5 nút 48dp trong 280dp, flip lời (scrimStrong). Library gõ 'rick' → nút xoá là IconButton, xoá về hint, field không cao hơn. Logcat không có `E/flutter`. Build: package_info_plus 9.0.1 fail `:package_info_plus:compileDebugKotlin` "Inconsistent JVM-target compatibility (Java 17 vs Kotlin 11)" vì root `android/build.gradle.kts` ép jvmTarget 11 cho mọi subproject; hạ ^8.3.1 vẫn lỗi (cùng nguyên nhân, khác connectivity_plus ở thứ tự evaluate) → thêm `kotlin.jvm.target.validation.mode=warning` vào `android/gradle.properties` (module không có Java source nên chỉ là cảnh báo). Giữ ^8.3.1 (đủ cho `PackageInfo.fromPlatform`). CHƯA test thủ công: sheet Sửa thông tin (Enter → sang ô Nghệ sĩ, Done → Lưu), nút info Shuffle Loop / FAB tạo playlist / nút gỡ bài (playlist_screen), 'Chọn tất cả' của picker, 'Thử lại tất cả' của summary, error state init thư mục (khó tái hiện trên emulator), TalkBack đọc label/hint. Build vẫn cần python-build-standalone 3.13 + `buildPython(...)` tạm trong gradle (đã revert trước commit). Emulator crash 1 lần khi `am start` ngay sau `force-stop` (lỗi framework "Activity client record must not be null"), chạy lại OK; hộp "Try out your stylus" của hệ thống nuốt `adb input text` lần đầu — bấm Cancel rồi gõ lại.
