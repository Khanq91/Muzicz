...
**Checklist:**
- [ ] `layering_violation` — `lib/features/downloader/screens/playlist_picker/playlist_picker_screen.dart:58` (medium) — Feature downloader đã thiết lập pattern gateway qua Riverpod (`analyzeGatewayProvider`/`downloadGatewayProvider` bọc `YtdlpService…
- [ ] `logic_in_presentation` — `lib/features/downloader/screens/format/format_screen.dart:248` (medium) — `_FormatScreenState._startDownload` chứa toàn bộ nghiệp vụ bắt đầu tải: lưu output path, xin quyền notification theo platform, map…
- [ ] `missing_autodispose` — `lib/features/downloader/providers/download_provider.dart:480` (medium) — Provider.family không autoDispose: mỗi taskId từng được watch sẽ giữ một instance sống suốt vòng đời app, không bao giờ được giải …
